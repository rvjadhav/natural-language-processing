...
class WordPredictorTesterGUI extends JFrame implements KeyListener { 
	...
	BigramCompute bgramComp;
    Trie tr = new Trie();
    boolean space = false;
	public WordPredictorTesterGUI() {
		bgramComp = new BigramCompute();
	...
	public void keyPressed(KeyEvent e) {
		...
		if ( index == -2){
	        ...
	        suggestion = bgramComp.countBigram(partialWord);
	        if(suggestion.size() > 9) {
	        	suggestion = new ArrayList<String>(suggestion.subList(0, 9));
	            }
	            
	            if(suggestion.size() > 0)
	               input.setText(arrtoString(suggestion) );
		...
		if ( index == -1 ) {
			...
			if ( Character.isAlphabetic(ch) && inWord ) {
				...
				// you have to search the prefix tree, by sending partialWord to the tree and returns 
				// up to nine most popular words start with partialWord.( i.e. popular array is populated by a trie method)
            if(!tr.hasDictionary())
            {
               try {
                  long then = System.currentTimeMillis();
                  tr.insertDictionary("/home/rahul/IdeaProjects/natural-language-processing/word-predictor/res/dictionary.txt"); // insert dictionary
			...
			// if the key pressed is enter or space or numbers
			//System.out.println(index);
			if(space){
				output.setEditable(false);
				if(suggestion != null){
					if(suggestion.get(index)!=null){
						current = output.getText();
						current += suggestion.get(index) + " ";
						output.setText(current);
						suggestion = bgramComp.countBigram(suggestion.get(index));
				        if(suggestion.size() > 9) {
				        	suggestion = new ArrayList<String>(suggestion.subList(0, 9));
				            }
